...
- **重要**: LocalStorageはブラウザ（デバイス）ごとに独立して保存されます
  - PCとスマホは別々のデータとして管理されます
  - PCで社員を削除しても、スマホのブラウザには残ります
  - 各デバイスで個別に設定・削除を行う必要があります
...
#### サイトが正しく表示されない場合
1. **`.nojekyll`ファイルを作成**: リポジトリのルートに`.nojekyll`という空ファイルを作成（Jekyllの処理を無効化）
2. **ファイルパスの確認**: すべてのパスが相対パス（`style.css`、`admin.html`など）になっているか確認
3. **ブラウザのキャッシュをクリア**: 古いキャッシュが原因の可能性があります
4. **ファイルがコミットされているか確認**: すべてのファイル（HTML、CSS、JS）がリポジトリに含まれているか確認

#### よくある問題
- **CSSが読み込まれない**: ファイルパスが正しいか、`.nojekyll`ファイルが存在するか確認
- **JavaScriptが動作しない**: ブラウザのコンソールでエラーを確認
- **リンクが機能しない**: 相対パスが正しいか確認（`admin.html`など）

### 注意事項
- GitHub Pagesは静的サイトのみサポート（サーバーサイド処理は不可）
- LocalStorageはブラウザごとに独立して保存されます
- HTTPSで配信されるため、一部の機能（Clipboard APIなど）が制限される場合があります
